...
\begin{defn}
  Pentru a calcula elementele secventei $b$, se foloseste algoritmul
  \textbf{Berlekamp Massey} daca $|F|$ este un numar prim, altfel, se factorizeaza
  $|F| = p_{1}^{k_{1}} p_{2}^{k_{2}} \ldots p_{t}^{k_{t}}$ si se rezolva $t$
  probleme cu $|F_{i}'| = p_{i}^{k_{i}}$ cu algoritmul \textbf{Reeds-Sloane}, iar
  rezultatele se combina folosind teorema chineza a resturilor.
\end{defn}
